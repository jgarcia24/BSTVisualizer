...
/******************** printPreOrder **************************************
Purpose:
 * Prints the tree's values pre order
Parameters:
 * I    nodeT *p    the root of the tree
Returns:
 * N/A
...
/******************** printPostOrder **************************************
Purpose:
 * Prints the tree's values post order
Parameters:
 * I    nodeT *p    the root of the tree
Returns:
 * N/A
...
int getHeight(nodeT *p)
{
  ...
  leftSubTreeMaxHeight = getHeight(p->pLeft);
  rightSubTreeMaxHeight = getHeight(p->pRight);
  
  if(leftSubTreeMaxHeight > rightSubTreeMaxHeight)
    {
      return leftSubTreeMaxHeight + 1;
    }
  else
    {
      return rightSubTreeMaxHeight + 1;
    }
}
...
int getNumberOfNodes(nodeT *p)
{
  ...
  {
    count = count + getNumberOfNodes(p->pLeft);
    count = count + getNumberOfNodes(p->pRight);
  }
...
int contains(nodeT* p, int key)
{
  if(p == NULL)
  {
    return 1; //empty tree, false
  }
  else
  {
    if(key == p->value)
    {
      return 0; //key found, true
    }
    else if(key < p->value)
    {
      return contains(p->pLeft, key); //searches left subtree
    }
    else if(key > p->value)
    {
      return contains(p->pRight, key); //searches right subtree
    }
  }
}
...
void treeToSortedArray(nodeT* p, int sortedArray[], int i)
{
  ...
  {
    treeToSortedArray(p->pLeft, sortedArray, i);
    sortedArray[i++] = p->value;
    treeToSortedArray(p->pRight, sortedArray, i);
  }
}
...
nodeT* arrayToBalancedTree(int sortedArray[], int start, int end)
{
  ...
  nodeT* bRoot = newNodeT(sortedArray[mid]);
  
  bRoot->pLeft = arrayToBalancedTree(sortedArray, start, mid - 1);
  bRoot->pRight = arrayToBalancedTree(sortedArray, mid + 1, end);
